...
#   GTEST_HOME - where GTest home dir found
#   TARGET_PLATFORM - Influences how the outputs are generated by modifying the
#       GTEST_OBJS_DIR and GTEST_LIB_DIR to use a sub-directory under the
#       normal objs and lib directories.  Also modifies where to search for the
#       GTEST_LIB to link against.
#   GTEST_OBJS_DIR - a directory where o and d files go
#   GTEST_LIB_DIR - a directory where libs go
#   GTEST_ENABLE_DEBUG - build for debug
#   GTEST_USE_MEM_LEAK_DETECTION - Links with overridden new and delete
#   GTEST_USE_STD_CPP_LIB - Set to N to keep the standard C++ library out
#		of the test harness
#   GTEST_USE_GCOV - Turn on coverage analysis
#		Clean then build with this flag set to Y, then 'make gcov'
#   GTEST_MAPFILE - generate a map file
#   GTEST_WARNINGFLAGS - overly picky by default
#   OTHER_MAKEFILE_TO_INCLUDE - a hook to use this makefile to make
#		other targets. Like CSlim, which is part of fitnesse
#   GTEST_USE_VPATH - Use Make's VPATH functionality to support user
...
#	GTEST_CXXFLAGS - flags for the C++ compiler
#	GTEST_CPPFLAGS - flags for the C++ AND C preprocessor
#	GTEST_CFLAGS - flags for the C complier
#	GTEST_LDFLAGS - Linker flags
#----------
...
# Debug on by default
ifndef GTEST_ENABLE_DEBUG
	GTEST_ENABLE_DEBUG = N
endif

# new and delete for memory leak detection on by default
ifndef GTEST_USE_MEM_LEAK_DETECTION
	GTEST_USE_MEM_LEAK_DETECTION = N
endif

# Use the standard C library
ifndef GTEST_USE_STD_C_LIB
	GTEST_USE_STD_C_LIB = Y
endif

# Use the standard C++ library
ifndef GTEST_USE_STD_CPP_LIB
	GTEST_USE_STD_CPP_LIB = Y
endif

# Use long long, off by default
ifndef GTEST_USE_LONG_LONG
	GTEST_USE_LONG_LONG = N
endif

ifndef GTEST_PEDANTIC_ERRORS
	GTEST_PEDANTIC_ERRORS = Y
endif

# Default warnings
ifndef GTEST_WARNINGFLAGS
	GTEST_WARNINGFLAGS =  -Wall -Wextra -Werror -Wshadow -Wswitch-default -Wswitch-enum -Wconversion -Wno-long-long
ifeq ($(GTEST_PEDANTIC_ERRORS), Y)
	GTEST_WARNINGFLAGS += -pedantic-errors
endif
ifeq ($(UNAME_OS),$(LINUX_STR))
	GTEST_WARNINGFLAGS += -Wsign-conversion
endif
	GTEST_CXX_WARNINGFLAGS = -Woverloaded-virtual
	GTEST_C_WARNINGFLAGS = -Wstrict-prototypes
endif
...
# -Wno-global-constructors Wno-exit-time-destructors -> Great warnings, but in GTest it is impossible to avoid as the automatic test registration depends on the global ctor and dtor
...
# -Wno-c++11-long-long -> When it detects long long, then we can use it and no need for a warning about that
	GTEST_CXX_WARNINGFLAGS += -Weverything -Wno-disabled-macro-expansion -Wno-padded -Wno-global-constructors -Wno-exit-time-destructors -Wno-weak-vtables -Wno-old-style-cast -Wno-c++11-long-long
	GTEST_C_WARNINGFLAGS += -Weverything -Wno-padded
...
# -Wno-reserved-id-macro -> Many GTest macros start with __, which is a reserved namespace
# -Wno-keyword-macro -> GTest redefines the 'new' keyword for memory leak tracking
	GTEST_CXX_WARNINGFLAGS += -Wno-reserved-id-macro -Wno-keyword-macro
	GTEST_C_WARNINGFLAGS += -Wno-reserved-id-macro -Wno-keyword-macro
endif
endif

# Uhm. Maybe put some warning flags for SunStudio here?
ifeq ($(COMPILER_NAME),$(SUNSTUDIO_CXX_STR))
	GTEST_CXX_WARNINGFLAGS =
	GTEST_C_WARNINGFLAGS =
endif

# Default dir for temporary files (d, o)
ifndef GTEST_OBJS_DIR
ifndef TARGET_PLATFORM
    GTEST_OBJS_DIR = objs
else
    GTEST_OBJS_DIR = objs/$(TARGET_PLATFORM)
endif
endif

# Default dir for the outout library
ifndef GTEST_LIB_DIR
    GTEST_LIB_DIR = lib
endif

# No map by default
ifndef GTEST_MAP_FILE
	GTEST_MAP_FILE = N
endif

# No extentions is default
ifndef GTEST_USE_EXTENSIONS
	GTEST_USE_EXTENSIONS = N
endif

# No VPATH is default
ifndef GTEST_USE_VPATH
	GTEST_USE_VPATH := N
endif
# Make empty, instead of 'N', for usage in $(if ) conditionals
ifneq ($(GTEST_USE_VPATH), Y)
	GTEST_USE_VPATH :=
endif
...
ifeq ($(GTEST_USE_STD_C_LIB), N)
	GTEST_USE_STD_CPP_LIB = N
	GTEST_USE_MEM_LEAK_DETECTION = N
	GTEST_CPPFLAGS += -DGTEST_STD_C_LIB_DISABLED
	GTEST_CPPFLAGS += -nostdinc
endif

ifeq ($(GTEST_USE_LONG_LONG), Y)
	GTEST_CPPFLAGS += -DGTEST_USE_LONG_LONG
endif

ifeq ($(GTEST_ENABLE_DEBUG), Y)
	GTEST_CXXFLAGS += -g
	GTEST_CFLAGS += -g
	GTEST_LDFLAGS += -g
endif

ifeq ($(GTEST_USE_STD_CPP_LIB), N)
	GTEST_CPPFLAGS += -DGTEST_STD_CPP_LIB_DISABLED
ifeq ($(GTEST_USE_STD_C_LIB), Y)
	GTEST_CXXFLAGS += -nostdinc++
endif
endif
GMOCK_HOME = /usr/local
GTEST_HOME = $(GMOCK_HOME)
GTEST_CPPFLAGS += -I$(GMOCK_HOME)/include/gmock
GMOCK_LIBRARY = $(GMOCK_HOME)/lib64/libgmock.a
LD_LIBRARIES += $(GMOCK_LIBRARY)
GTEST_CPPFLAGS += -DGTEST_INCLUDE_GTEST_TESTS
GTEST_WARNINGFLAGS =
GTEST_CPPFLAGS += -I$(GTEST_HOME)/include/gtest -I$(GTEST_HOME)
GTEST_LIBRARY = $(GTEST_HOME)/lib64/libgtest.a
LD_LIBRARIES += $(GTEST_LIBRARY)
GTEST_MAIN_LIBRARY = $(GTEST_HOME)/lib64/libgtest_main.a
LD_LIBRARIES += $(GTEST_MAIN_LIBRARY)

GTEST_CXXFLAGS += $(GTEST_WARNINGFLAGS) $(GTEST_CXX_WARNINGFLAGS)
GTEST_CPPFLAGS += $(GTEST_WARNINGFLAGS)
GTEST_CXXFLAGS += $(GTEST_MEMLEAK_DETECTOR_NEW_MACRO_FILE)
GTEST_CPPFLAGS += $(GTEST_MEMLEAK_DETECTOR_MALLOC_MACRO_FILE)
GTEST_CFLAGS += $(GTEST_C_WARNINGFLAGS)

ifndef TARGET_PLATFORM
GTEST_LIB_LINK_DIR = $(GTEST_HOME)/lib64
else
GTEST_LIB_LINK_DIR = $(GTEST_HOME)/lib/$(TARGET_PLATFORM)
endif

# Link with GTest lib
# GTEST_LIB = $(GTEST_LIB_LINK_DIR)/libgtest_main.a

ifdef GTEST_STATIC_REALTIME
	LD_LIBRARIES += -lrt
endif

TARGET_LIB = \
    $(GTEST_LIB_DIR)/lib$(COMPONENT_NAME).a
...
__src_to = $(subst .c,$1, $(subst .cc,$1, $(subst .cpp,$1,$(if $(GTEST_USE_VPATH),$(notdir $2),$2))))
src_to = $(addprefix $(GTEST_OBJS_DIR)/,$(call __src_to,$1,$2))
...
# Add the component name to the objs dir path, to differentiate between same-name objects
	GTEST_OBJS_DIR := $(addsuffix /$(COMPONENT_NAME),$(GTEST_OBJS_DIR))
endif


#The gcda files for gcov need to be deleted before each run
#To avoid annoying messages.
RUN_TEST_TARGET = ./$(TEST_TARGET) $(GTEST_EXE_FLAGS)
...
# We'll use the GTEST_CFLAGS etc so that you can override AND add to the GTest flags
CFLAGS = $(GTEST_CFLAGS) $(GTEST_ADDITIONAL_CFLAGS)
CPPFLAGS = $(GTEST_CPPFLAGS) $(GTEST_ADDITIONAL_CPPFLAGS)
CXXFLAGS = $(GTEST_CXXFLAGS) $(GTEST_ADDITIONAL_CXXFLAGS)
LDFLAGS = $(GTEST_LDFLAGS) $(GTEST_ADDITIONAL_LDFLAGS)
...
TEST_DEPS = $(TEST_OBJS) $(MOCKS_OBJS) $(PRODUCTION_CODE_START) $(TARGET_LIB) $(USER_LIBS) $(PRODUCTION_CODE_END) $(STDLIB_CODE_START)
test-deps: $(TEST_DEPS)
...
clean:
	...
	$(SILENCE)rm -rf gcov $(GTEST_OBJS_DIR)
